...
class BasicModel(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        ...
        if self.current_epoch>epochs*0.01:
            ...
            x1 = self.train_set[idx1][0] # x1 = self.train_set[idx1][0].cuda()
            x2 = self.train_set[idx2][0] # x2 = self.train_set[idx2][0].cuda()
    ...
    def validation_epoch_end(self, outputs):
        ...
        # Select only the first images from the two pairs.
        x1 = self.val_set[idx1][0]# x1 = self.val_set[idx1][0].cuda()
        x2 = self.val_set[idx2][0]# x2 = self.val_set[idx2][0].cuda()
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:
            ...
            idx2 = random.randint(0, len(self.val_set) - 1)
            # Select only the first images from the two pairs.
            x1_rand = self.val_set[idx1][0] # x1_rand = self.val_set[idx1][0].cuda()
            x2_rand = self.val_set[idx2][0]# x2_rand = self.val_set[idx2][0].cuda()
    ...
    def setup(self, stage=None):
        ...
        if dataset_name in ['shapes3d_custom_detangle', 'cifar', 'svhn']:
