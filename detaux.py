...
def train(args):
    ...
    if args.wandb:
        ...
        trainer = Trainer.from_argparse_args(args, devices=[args.gpu_idx], accelerator="cpu", logger=wandb_logger, callbacks=[checkpoint_callback], deterministic=True)
    elif args.save_dir:
        # Without 16 bit precision.
        trainer = Trainer.from_argparse_args(args, devices=[args.gpu_idx], accelerator="cpu", callbacks=[checkpoint_callback], deterministic=True)
    else:
        trainer = Trainer.from_argparse_args(args, devices=[args.gpu_idx], accelerator="cpu", deterministic=True)
